...
function onDicEx()
{
  location.href = "<%=rootPath%>/identify.do?method=toCreateDicFile&txtXML=MANAGEUNIT&txtNextUrl=<%=strSelfPath%>/manageunitlist.jsp";
}
...
function onEditEx()
{
  ...
  Efs.Common.ajax("<%=rootPath%>/ajax?method=getMunitDetail&txtUnitID=" + sUnitID,"",function(succ,response,options){
     if(succ){ // 是否成功
...
<div id="unitgrid" region="center" xtype="grid" title="" border="false" pagingBar="true" pageSize="25" onEfsRowClick="doGridClick()" buttonAlign="center">
  <div xtype="tbar">
    ...
    <div iconCls="icon-edit" id="cmdEdit" text="编辑单位#E" onEfsClick="onEditEx()" disabled></div>
    <div iconCls="icon-del" id="cmdDel" text="删除单位#D" onEfsClick="onDelEx()" disabled></div>
	...
	<div id="unitList" xtype="store" url="<%=rootPath%>/ajax?method=getRsQryMunitList" txtXML='' autoLoad="false">
	...
	<div xtype="colmodel">
		...
		<div header="单位级别" width="60" sortable="true" dataIndex="MLEVEL" align="center"></div>
    ...
    <div iconCls="icon-panel" id="UnitMWin" xtype="window" width="560" height="250" title="添加用户" resizable="true" modal="true">
      <div region="center" xtype="panel" title="" border="false" autoScroll="true">
        ...
        <form txtXML="" id="frmData" class="efs-box" method="post" url="<%=rootPath%>/ajax?method=unitDeal" onEfsSuccess="frmPostSubBack(true)" onEfsFailure="frmPostSubBack(false)">
