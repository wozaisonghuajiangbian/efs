...
function onEditEx()
{
  ...
  Efs.Common.ajax("<%=rootPath%>/ajax?method=getUserDetail&txtUserID=" + sUserID,"",function(succ,response,options){
     if(succ){ // 是否成功
...
function onDelEx()
{
  ...
  {
    setAttribute("url", "<%=rootPath%>/identify.do?method=userDrop");
...
function onPswEx()
{
  with(document.frmPost)
  {
    action = "<%=rootPath%>/identify.do?method=userSetPassword";
...
function onDicEx()
{
  location.href = "<%=rootPath%>/identify.do?method=toCreateDicFile&txtXML=USERLIST&txtNextUrl=<%=strSelfPath%>/userlist.jsp";
}
...
{ 
  with(Efs.getDom("frmData"))
  {
    if(Efs.getDom("UserID").getAttribute("operation") == "0")
      setAttribute("url", "<%=rootPath%>/ajax?method=userAdd");
    else if(Efs.getDom("UserID").getAttribute("operation") == "1")
      setAttribute("url", "<%=rootPath%>/ajax?method=userEdit");
  }
...
<div id="dicgrid" region="center" xtype="grid" border="false" pagingBar="true" pageSize="25" onEfsRowClick="doGridClick()" buttonAlign="center">
  <div xtype="tbar">
    ...
    <div text="-"></div>
    <div iconCls="icon-key" id="cmdPsw" text="初始化口令#P" onEfsClick="onPswEx()" disabled></div>
    <div text="-"></div>
	...
	<div id="dicList" xtype="store" url="<%=rootPath%>/ajax?method=getRsQryUserList" txtXML='' autoLoad="false">
